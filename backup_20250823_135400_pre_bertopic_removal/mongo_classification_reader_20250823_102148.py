...
class MongoClassificationReader:
    ...
    def get_collection_name(self, tenant_name: str = None) -> str:
        ...
        if effective_tenant:
            # Recupera tenant_info completo (slug + id)
            tenant_info = self.get_tenant_info_from_name(effective_tenant)
            
            if tenant_info:
                tenant_slug = tenant_info['tenant_slug']
                tenant_id = tenant_info['tenant_id']
                # Pattern robusto: {tenant_slug}_{tenant_id}
                return f"{tenant_slug}_{tenant_id}"
            else:
                # Fallback per compatibilità con tenant non in MySQL
                print(f"⚠️ Tenant '{effective_tenant}' non trovato in MySQL, uso nome come fallback")
                safe_tenant_name = effective_tenant.replace("-", "_").replace(" ", "_").lower()
                return f"{safe_tenant_name}_classifications"
        else:
