    ...
    def classifica_e_salva_sessioni(self,
        ...
        for i, (session_id, prediction) in enumerate(zip(session_ids, predictions)):
            ...
            try:
                ...
                if use_ensemble and prediction:
                ...
                if success:
                    ...
                    # 🆕 SALVA ANCHE IN MONGODB CON DETTAGLI ML/LLM COMPLETI
                    try:
                        # Estrai dettagli ML e LLM dal prediction
                        ml_result = prediction.get('ml_prediction')
                        llm_result = prediction.get('llm_prediction')
                        
                        # Calcola disagreement se entrambi disponibili
                        ml_label = ml_result.get('predicted_label', '') if ml_result else ''
                        llm_label = llm_result.get('predicted_label', '') if llm_result else ''
                        has_disagreement = (ml_label and llm_label and ml_label != llm_label)
                        
                        # Calcola disagreement confidence (differenza confidenze)
                        ml_conf = float(ml_result.get('confidence', 0.0)) if ml_result else 0.0
                        llm_conf = float(llm_result.get('confidence', 0.0)) if llm_result else 0.0
                        confidence_disagreement = abs(ml_conf - llm_conf) if (ml_conf > 0 and llm_conf > 0) else 0.0
                        
                        # Salva classificazione completa in MongoDB
                        mongo_success = self.mongo_reader.save_classification_result(
                            session_id=session_id,
                            client_name=self.tenant_slug,
                            ml_result=ml_result,  # Dettagli ML completi
                            llm_result=llm_result,  # Dettagli LLM completi
                            final_decision={
                                'predicted_label': predicted_label,
                                'confidence': confidence,
                                'method': method,
                                'reasoning': f"Ensemble method={method}, agreement={not has_disagreement}"
                            },
                            conversation_text=sessioni[session_id].get('testo_completo', ''),
                            needs_review=False  # Per ora auto-classifica tutto
                        )
                        
                        if not mongo_success and i < 10:  # Log solo per prime 10 se ci sono errori
                            print(f"   ⚠️ Warning: MongoDB save failed for session {i+1}")
                            
                    except Exception as mongo_error:
                        if i < 5:  # Log solo prime 5 per non spammare
                            print(f"   ❌ MongoDB save error for session {i+1}: {mongo_error}")
