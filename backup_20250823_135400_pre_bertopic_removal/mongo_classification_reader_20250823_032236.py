...
class MongoClassificationReader:
    ...
    def __init__(self, mongodb_url: str = "mongodb://localhost:27017", 
        ...
        if tenant_name:
            self.collection_name = f"{tenant_name}_classifications"
    ...
    def get_tenant_id_from_name(self, tenant_name: str) -> Optional[str]:
        ...
        try:
            ...
            cursor.execute("SELECT tenant_id FROM tenants WHERE tenant_name = %s AND is_active = 1", (tenant_name,))
    ...
    def get_collection_name(self, tenant_name: str = None) -> str:
        ...
        Returns:
            Nome della collection per il tenant basata su tenant_id
        ...
        if effective_tenant:
            ...
            if tenant_id:
                # Usa tenant_id per collection name (univoco e stabile)
                return f"{tenant_id}_classifications"
            else:
