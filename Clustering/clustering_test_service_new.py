...
class ClusteringTestService:
    ...
    def _get_pipeline(self, tenant_slug: str) -> EndToEndPipeline:
        """
        Ottiene pipeline per il tenant specificato, utilizzando cache
        
        Args:
            tenant_slug: Slug del tenant (es: 'humanitas')
            
        Returns:
            Istanza EndToEndPipeline per il tenant
        """
        if tenant_slug not in self.pipeline_cache:
            try:
                ...
                self.pipeline_cache[tenant_slug] = pipeline
                logging.info(f"✅ Pipeline {tenant_slug} inizializzata e cached")
            except Exception as e:
                logging.error(f"❌ Errore inizializzazione pipeline {tenant_slug}: {e}")
                raise RuntimeError(f"Impossibile inizializzare pipeline per {tenant_slug}: {e}")
        
        return self.pipeline_cache[tenant_slug]
