...
class PromptManager:
    ...
    def _resolve_tenant_id(self, tenant_input: str) -> Optional[str]:
        """
        Risolve tenant_slug o tenant_name in tenant_id UUID
        
        Args:
            tenant_input: Può essere tenant_id UUID, tenant_slug, o tenant_name
            
        Returns:
            tenant_id UUID o None se non trovato
        """
        if not tenant_input:
            return None
        
        # Se sembra già un UUID, usa direttamente
        if len(tenant_input) == 36 and tenant_input.count('-') == 4:
            return tenant_input
        
        # Altrimenti cerca nella tabella tenants
        if not self.connect():
            return None
        ...
        try:
            ...
            # Prima cerca per tenant_slug, poi per tenant_name (case insensitive)
            query = """
            SELECT tenant_id 
            FROM tenants 
            WHERE tenant_slug = %s OR LOWER(tenant_name) = LOWER(%s)
            AND is_active = 1
            LIMIT 1
            """
            
            cursor.execute(query, (tenant_input, tenant_input))
            result = cursor.fetchone()
            cursor.close()
            
            if result:
                tenant_id = result[0]
                self.logger.debug(f"🔍 Risolto tenant '{tenant_input}' → '{tenant_id}'")
                return tenant_id
            
            self.logger.warning(f"⚠️ Tenant non trovato: '{tenant_input}'")
            return None
            
        except Error as e:
            self.logger.error(f"❌ Errore risoluzione tenant: {e}")
            return None
    ...
    def get_prompt(self, 
        ...
        Args:
            tenant_id: ID del tenant (può essere slug o UUID)
        ...
        """
        try:
            # Risolvi tenant_slug in tenant_id UUID se necessario
            resolved_tenant_id = self._resolve_tenant_id(tenant_id)
            if not resolved_tenant_id:
                self.logger.warning(f"Tenant non trovato: {tenant_id}")
                return None
            
            # Cache key per il prompt (usa l'UUID risolto)
            ...
            if self._is_cached_valid(cache_key):
                ...
                # Carica dal database
                prompt_data = self._load_prompt_from_db(resolved_tenant_id, engine, prompt_type, prompt_name)
                if not prompt_data:
