...
# Configurazione CORS specifica per VS Code tunneling
CORS(app, 
     origins="*",
...
# Hook aggiuntivo per VS Code tunneling - garantisce headers su tutte le risposte
@app.after_request
def after_request_cors_fix(response):
...
class ClassificationService:
    ...
    def __init__(self):
        ...
        self.mongo_readers = {}  # Cache dei MongoClassificationReader per tenant
    ...
    def get_mongo_reader(self, tenant_name: str) -> 'MongoClassificationReader':
        """
        Ottieni o crea MongoClassificationReader specifico per tenant
        ...
        """
        if tenant_name not in self.mongo_readers:
            print(f"🔧 Inizializzazione MongoReader per tenant: {tenant_name}")
            self.mongo_readers[tenant_name] = MongoClassificationReader(tenant_name=tenant_name)
            print(f"✅ MongoReader per tenant {tenant_name} inizializzato")
        return self.mongo_readers[tenant_name]
    ...
    def get_pipeline(self, client_name: str) -> EndToEndPipeline:
        ...
        Args:
            client_name: Nome del cliente (es. 'humanitas' o 'Fate Bene Fratelli')
            
        Returns:
            Pipeline configurata per il cliente
        """
        # 🔍 RISOLUZIONE TENANT SLUG: Mappa nome cliente a tenant_slug
        from TagDatabase.tag_database_connector import TagDatabaseConnector
        tag_connector = TagDatabaseConnector()
        try:
            resolved_tenant_slug = tag_connector.resolve_tenant_slug(client_name)
            if resolved_tenant_slug:
                effective_client_name = resolved_tenant_slug
                if client_name != effective_client_name:
                    print(f"🔄 Mappatura tenant: '{client_name}' → '{effective_client_name}'")
            else:
                print(f"⚠️  Tenant '{client_name}' non trovato nel database, uso nome originale")
                effective_client_name = client_name
        except Exception as e:
            print(f"❌ Errore risoluzione tenant: {e}, uso nome originale")
            effective_client_name = client_name
        finally:
            tag_connector.disconnetti()
        
        # Crea lock specifico per questo cliente se non esiste
        if effective_client_name not in self._pipeline_locks:
            with self._global_init_lock:
                if effective_client_name not in self._pipeline_locks:
                    self._pipeline_locks[effective_client_name] = threading.Lock()
        
        # Usa lock specifico del cliente
        with self._pipeline_locks[effective_client_name]:
            if effective_client_name not in self.pipelines:
                print(f"🔧 Inizializzazione pipeline per cliente: {effective_client_name} (con lock)")
                ...
                pipeline = EndToEndPipeline(
                    tenant_slug=effective_client_name,
                ...
                self.pipelines[effective_client_name] = pipeline
                print(f"✅ Pipeline {effective_client_name} inizializzata")
                
            return self.pipelines[effective_client_name]
...
def api_get_review_cases(client_name: str):
    ...
    try:
        ...
        # Ottieni reader MongoDB specifico per tenant
...
def get_tenants():
    ...
    try:
        ...
        # Per get_available_tenants() non serve specificare tenant (legge da MySQL)
...
def get_all_sessions(client):
    ...
    try:
        ...
        # Risolvi il tenant_slug corretto per il client
        from TagDatabase.tag_database_connector import TagDatabaseConnector
        tag_connector = TagDatabaseConnector()
        resolved_tenant_slug = tag_connector.resolve_tenant_slug(client)
        
        print(f"🔗 Client '{client}' → Tenant slug: '{resolved_tenant_slug}'")
        
        # Estrai tutte le sessioni dal database SENZA pipeline usando il tenant corretto
        from Preprocessing.session_aggregator import SessionAggregator
        aggregator = SessionAggregator(schema=resolved_tenant_slug)
...
def api_get_all_tenants():
    ...
    try:
        ...
        # Usa MongoDB reader per recuperare dati aggregati (non specifico per tenant)
        mongo_reader = MongoClassificationReader()
...
def api_get_labels_by_tenant(tenant_name: str):
    ...
    try:
        ...
        # Usa MongoDB reader specifico per tenant
...
@app.route('/api/prompts/<tenant_id>', methods=['GET'])
...
@app.route('/api/tools/<tenant_id>', methods=['GET'])
def get_tools_for_tenant(tenant_id):
    """
    ...
    Returns:
        JSON con lista dei tools
        
    Data ultima modifica: 2025-08-22
    """
    try:
        sys.path.append(os.path.join(os.path.dirname(__file__), 'Utils'))
...
def get_tool_by_id(tool_id):
    """
    Recupera un singolo tool per ID
    
    Args:
        tool_id: ID del tool
        
    Returns:
        JSON con dati del tool
    ...
    try:
        ...
        # Per ora non implementato nel ToolManager, ritorna errore
        return jsonify({
...
def create_tool():
    ...
    Body:
        tool_name: Nome identificativo del tool
        display_name: Nome visualizzato
        description: Descrizione del tool
        function_schema: Schema della funzione (JSON)
        tenant_id: ID del tenant
        tenant_name: Nome del tenant
        is_active: Se il tool è attivo (default True)
        
    Returns:
        JSON con risultato creazione
    ...
    try:
        ...
        # Validazione campi obbligatori
        required_fields = ['tool_name', 'display_name', 'description', 'function_schema']
        for field in required_fields:
            if field not in data:
...
def update_tool(tool_id):
    ...
    try:
        ...
        tool_manager = ToolManager()
        result = tool_manager.update_tool(tool_id, **data)
...
def delete_tool(tool_id):
    """
    Elimina (disattiva) un tool
    
    Args:
        tool_id: ID del tool da eliminare
        
    Returns:
        JSON con risultato eliminazione
...
@app.route('/api/tools/summary/<tenant_id>', methods=['GET'])
def get_tools_summary(tenant_id):
    """
    Recupera riepilogo tools per un tenant
    
    Args:
        tenant_id: UUID del tenant
        
    Returns:
        JSON con statistiche sui tools
    ...
    try:
        ...
        tool_manager = ToolManager()
        summary = tool_manager.get_available_tools_summary(tenant_id)
        
        return jsonify(summary)
        
    except Exception as e:
        print(f"❌ Errore riepilogo tools per tenant {tenant_id}: {e}")
        return jsonify({
            'error': str(e),
            'tenant_id': tenant_id,
            'total_tools': 0
