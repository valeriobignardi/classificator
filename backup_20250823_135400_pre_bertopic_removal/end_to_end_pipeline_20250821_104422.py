    ...
    def _try_load_latest_model(self):
        ...
        try:
            ...
            if self.bertopic_config.get('enabled', False) and _BERTopic_AVAILABLE and os.path.isdir(provider_dir):
                try:
                    print(f"🔄 Tentativo caricamento BERTopic provider da {provider_dir}")
                    ...
                    if provider is not None:
                        ...
                        print("⚠️ BERTopic provider caricamento fallito (modello corrotto rimosso)")
                        print("💡 Il sistema creerà un nuovo modello BERTopic al prossimo training")
                        
                except Exception as e:
                    print(f"⚠️ Errore caricamento BERTopic provider: {e}")
                    print(f"⚠️ Tipo errore: {type(e).__name__}")
                    import traceback
                    print(f"⚠️ Stack trace: {traceback.format_exc()}")
                    print("⚠️ Continuando senza BERTopic provider...")
            else:
