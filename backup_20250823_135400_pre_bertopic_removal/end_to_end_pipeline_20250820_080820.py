...
class EndToEndPipeline:
    ...
    def estrai_sessioni(self, 
        ...
        Args:
            ...
            limit: Limite massimo di sessioni da estrarre
            
        Returns:
            Dizionario con le sessioni estratte
        """
        print(f"📊 Estrazione sessioni per {self.tenant_slug}...")
        print(f"🔍 Parametri: giorni_indietro={giorni_indietro}, limit={limit}")
        ...
        sessioni = self.aggregator.estrai_sessioni_aggregate(limit=limit)
        ...
        print(f"✅ Estratte {len(sessioni_filtrate)} sessioni valide")
    ...
    def esegui_pipeline_completa(self,
        ...
        Args:
            ...
            limit: Limite massimo di sessioni
        ...
        try:
            ...
            sessioni = self.estrai_sessioni(giorni_indietro=giorni_indietro, limit=limit)
    ...
    def esegui_training_interattivo(self,
        ...
        Utile per creare modelli supervisionati con feedback umano
        
        Args:
            giorni_indietro: Giorni di dati da processare per il training
            limit: Limite massimo di sessioni per il training
            
        Returns:
            Risultati del training interattivo
        """
        start_time = datetime.now()
        print(f"🎓 TRAINING INTERATTIVO")
        print(f"📅 Periodo: ultimi {giorni_indietro} giorni")
        print(f"🔢 Limite sessioni: {limit or 'Nessuno'}")
        print("-" * 50)
        
        try:
            # 1. Estrazione sessioni per training
            sessioni = self.estrai_sessioni(giorni_indietro=giorni_indietro, limit=limit)
            ...
            # 3. Training interattivo obbligatorio
            training_metrics = self.allena_classificatore(
                sessioni, cluster_labels, representatives, suggested_labels, 
            ...
            results = {
                ...
                'training_sessions': len(sessioni),
                'clustering_results': {
                    'n_clusters': len(set(cluster_labels)) - (1 if -1 in cluster_labels else 0),
                    'n_outliers': sum(1 for label in cluster_labels if label == -1)
                },
            ...
            print("-" * 50)
            print(f"✅ TRAINING INTERATTIVO COMPLETATO!")
            print(f"⏱️  Durata: {duration.total_seconds():.1f} secondi")
            print(f"📊 Sessioni training: {len(sessioni)}")
