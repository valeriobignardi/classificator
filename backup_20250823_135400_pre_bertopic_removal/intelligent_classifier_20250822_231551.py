...
class IntelligentClassifier:
    ...
    def __init__(self,
        ...
        # MongoClassificationReader per salvataggio tenant-aware
        self.mongo_reader = None
        try:
            from mongo_classification_reader import MongoClassificationReader
            if client_name:
                self.mongo_reader = MongoClassificationReader(tenant_name=client_name)
                if enable_logging:
                    print(f"🗄️ MongoClassificationReader inizializzato per tenant: {client_name}")
            else:
                if enable_logging:
                    print(f"⚠️ MongoClassificationReader non inizializzato: client_name mancante")
        except Exception as e:
            if enable_logging:
                print(f"⚠️ MongoClassificationReader non disponibile: {e}")
            self.mongo_reader = None
        
        # Mantieni mongo_connector per backward compatibility (deprecato)
        self.mongo_connector = None
        if MONGODB_AVAILABLE:
            try:
                self.mongo_connector = MongoDBConnector(config_path=config_path)
                if enable_logging:
                    print(f"🗄️ MongoDB connector inizializzato (legacy)")
            except Exception as e:
                if enable_logging:
        ...
        self.altro_discovery_similarity_threshold = self.config.get('pipeline', {}).get('altro_discovery_similarity_threshold', 0.9)
        ...
        # FIXME: Definire tenant_id prima di usarlo
    ...
    def _build_system_message(self, conversation_context: Optional[str] = None) -> str:
        ...
        if self.prompt_manager:
            try:
                ...
                # 🔍 DEBUG: Verifica caricamento prompt dal database
                if hasattr(self, 'logger'):
                    self.logger.info(f"🔍 DEBUG SYSTEM PROMPT LOADING:")
                    self.logger.info(f"  - Tenant ID: {self.tenant_id}")
                    self.logger.info(f"  - Engine: LLM")
                    self.logger.info(f"  - Type: SYSTEM")
                    self.logger.info(f"  - Name: intelligent_classifier_system")
                    self.logger.info(f"  - Variables: {list(variables.keys())}")
                    self.logger.info(f"  - Loaded successfully: {system_prompt is not None}")
                    if system_prompt:
                        self.logger.info(f"  - Prompt length: {len(system_prompt)} characters")
                        self.logger.info(f"  - Preview: {system_prompt[:200]}...")
...
Ragionamento: {ex["motivation"]}"""
                ...
                # 🔍 DEBUG: Verifica caricamento user prompt dal database
                if hasattr(self, 'logger'):
                    self.logger.info(f"🔍 DEBUG USER PROMPT LOADING:")
                    self.logger.info(f"  - Tenant ID: {self.tenant_id}")
                    self.logger.info(f"  - Engine: LLM")
                    self.logger.info(f"  - Type: TEMPLATE")
                    self.logger.info(f"  - Name: intelligent_classifier_user_template")
                    self.logger.info(f"  - Variables: {list(variables.keys())}")
                    self.logger.info(f"  - Loaded successfully: {user_prompt is not None}")
                    if user_prompt:
                        self.logger.info(f"  - Prompt length: {len(user_prompt)} characters")
                        self.logger.info(f"  - Preview: {user_prompt[:200]}...")
...
"""
        ...
        # 🔍 DEBUG: Log del prompt completo per analisi
        if hasattr(self, 'logger'):
            self.logger.info("🔍 DEBUG LLM PROMPT COMPLETO:")
            self.logger.info("="*80)
            self.logger.info(f"🏥 TENANT: {getattr(self, 'tenant_id', 'N/A')}")
            self.logger.info(f"📝 INPUT TEXT: {conversation_text[:100]}...")
            ...
            self.logger.info("🤖 PROMPT FINALE INVIATO AL MODELLO:")
            self.logger.info("-"*40)
    ...
    def _validate_and_fix_json(self, response_text: str) -> Tuple[bool, str]:
        ...
        """
        # 🔍 DEBUG: Inizio validazione JSON
        if hasattr(self, 'logger'):
            self.logger.info(f"🔍 DEBUG JSON VALIDATION START:")
            self.logger.info(f"📝 ORIGINAL TEXT: '{response_text}'")
            self.logger.info(f"📏 LENGTH: {len(response_text)}")
            self.logger.info(f"🎯 STRIPPED: '{response_text.strip()}'")
        
        try:
            # Test parsing diretto
            json.loads(response_text.strip())
            if hasattr(self, 'logger'):
                self.logger.info("✅ DIRECT JSON PARSE SUCCESS")
        ...
        if json_match:
            ...
            if hasattr(self, 'logger'):
                self.logger.info(f"🔧 JSON EXTRACTION ATTEMPT: '{json_candidate}'")
            try:
                json.loads(json_candidate)
                if hasattr(self, 'logger'):
                    self.logger.info("✅ EXTRACTED JSON PARSE SUCCESS")
                return True, json_candidate
            except json.JSONDecodeError as e:
                if hasattr(self, 'logger'):
                    self.logger.info(f"❌ EXTRACTED JSON PARSE FAILED: {e}")
        
        # Cerca pattern mancanti comuni
        fixes = [
            (r'(\w+):', r'"\1":', "Add quotes to keys"),  # Aggiungi quote alle chiavi
            (r':\s*([^",\{\}\[\]]+)(\s*[,\}])', r': "\1"\2', "Add quotes to values"),  # Quote ai valori
            (r',\s*}', '}', "Remove trailing comma before }"),  # Rimuovi virgole finali
        ]
        
        for pattern, replacement, desc in fixes:
            fixed = re.sub(pattern, replacement, cleaned)
            if hasattr(self, 'logger'):
                self.logger.info(f"🔧 TRYING FIX '{desc}': '{fixed[:200]}...'")
            try:
                json.loads(fixed)
                if hasattr(self, 'logger'):
                    self.logger.info(f"✅ FIX '{desc}' SUCCESS!")
                return True, fixed
            except json.JSONDecodeError as e:
                if hasattr(self, 'logger'):
                    self.logger.info(f"❌ FIX '{desc}' FAILED: {e}")
                continue
        
        if hasattr(self, 'logger'):
            self.logger.error(f"🚫 ALL JSON CORRECTION ATTEMPTS FAILED FOR: '{response_text[:200]}...'")
    ...
    def _parse_llm_response(self, response_text: str, conversation_text: str = "") -> Tuple[str, float, str, str]:
        ...
        # 🔍 DEBUG: Dettaglio completo del parsing
        if hasattr(self, 'logger'):
            self.logger.info("🔍 DEBUG PARSING JSON:")
            self.logger.info("-"*60)
            self.logger.info(f"📝 INPUT TEXT: '{response_text}'")
            self.logger.info(f"📏 INPUT LENGTH: {len(response_text)}")
        
        try:
            # Auto-validazione e correzione
            is_valid, corrected_json = self._validate_and_fix_json(response_text)
            
            # 🔍 DEBUG: Risultato validazione
            if hasattr(self, 'logger'):
                self.logger.info(f"✅ JSON VALID: {is_valid}")
                self.logger.info(f"🔧 CORRECTED JSON: '{corrected_json}'")
            
            if not is_valid:
                self.logger.warning("JSON non valido anche dopo correzione automatica")
                if hasattr(self, 'logger'):
                    self.logger.info("🚨 CALLING FALLBACK DUE TO INVALID JSON")
            ...
            try:
                # 🔍 DEBUG: Pre-call debug
                if hasattr(self, 'logger'):
                    self.logger.debug(f"🔍 CALLING _semantic_label_resolution with:")
                    self.logger.debug(f"  - predicted_label: '{predicted_label}'")
                    self.logger.debug(f"  - conversation_text: '{conversation_text[:100]}...'")
                ...
                # 🔍 DEBUG: Post-call debug
                if hasattr(self, 'logger'):
                    self.logger.debug(f"🔍 RETURNED from _semantic_label_resolution:")
                    self.logger.debug(f"  - resolved_label: '{resolved_label}'")
                    self.logger.debug(f"  - resolution_method: '{resolution_method}'")
                    self.logger.debug(f"  - semantic_confidence: {semantic_confidence}")
                    
            except Exception as semantic_error:
                # 🔍 DEBUG: Errore intercettato
                if hasattr(self, 'logger'):
                    self.logger.error(f"🚨 ERROR in _semantic_label_resolution: {semantic_error}")
                    self.logger.error(f"🚨 ERROR type: {type(semantic_error)}")
                    import traceback
    ...
    def _fallback_parse_response(self, response_text: str) -> Tuple[str, float, str, str]:
        ...
        if len(mentioned_labels) == 1:
            ...
            return predicted_label, confidence, motivation, predicted_label  # FIXED: 4 valori
        ...
        # Se il LLM ha menzionato multiple etichette, è ambiguo
            ...
            return "altro", 0.2, motivation, "altro"  # FIXED: 4 valori
        
        # Nessuna etichetta riconosciuta - fallback completo
        return "altro", 0.1, "Risposta LLM non interpretabile senza pattern rigidi", "altro"  # FIXED: 4 valori
    
    # ==================== CODICE MORTO - NON PIÙ UTILIZZATO ====================
    # NOTA: Le funzioni seguenti sono state sostituite dal sistema di risoluzione semantica
    # basato su embeddings e similarità coseno. Mantenute commentate per riferimento storico.
    
    # def _normalize_label(self, label: str) -> str:
    #     """
    #     DEPRECATED: Normalizza le etichette con correzione intelligente di errori comuni
    #     SOSTITUITO DA: _semantic_label_resolution() che usa embeddings + cosine similarity
    #     """
    ...
    #     """
    #     DEPRECATED: Calcola distanza di Levenshtein tra due stringhe
    #     SOSTITUITO DA: Cosine similarity con embeddings in _resolve_label_semantically()
    #     """
    #     if len(s1) < len(s2):
    ...
    def classify_with_motivation(self, 
        ...
        try:
            ...
            # 🔍 DEBUG: Raw response dal LLM
            if hasattr(self, 'logger'):
                self.logger.info("🔍 DEBUG RAW LLM RESPONSE:")
                self.logger.info("="*80)
                self.logger.info(f"📝 RAW RESPONSE: '{raw_response}'")
                self.logger.info(f"📏 LENGTH: {len(raw_response)} characters")
                self.logger.info(f"🔤 TYPE: {type(raw_response)}")
                self.logger.info(f"🎯 STRIPPED: '{raw_response.strip()}'")
                self.logger.info("="*80)
    ...
    def _save_to_mongodb(self, conversation_text: str, result: ClassificationResult) -> None:
        """
        Salva la classificazione in MongoDB usando MongoClassificationReader tenant-aware
        ...
        try:
            ...
            # Crea oggetti dei risultati per il nuovo formato
            final_decision = {
                "predicted_label": result.predicted_label,
                "confidence": result.confidence,
                "method": result.method,
            ...
            # Salva usando il nuovo metodo tenant-aware
            success = self.mongo_reader.save_classification_result(
                session_id=session_id,
                client_name=self.client_name,
                final_decision=final_decision,
                conversation_text=conversation_text,
                needs_review=False,  # IntelligentClassifier di solito non richiede review
                review_reason=None
            )
            
            if success:
                self.logger.debug(f"💾 Classificazione salvata in MongoDB (tenant-aware): {session_id[:8]} -> {result.predicted_label}")
            else:
                self.logger.warning(f"⚠️ Errore salvataggio MongoDB per tenant {self.client_name}")
    ...
    def _semantic_label_resolution(self, proposed_label: str, 
        ...
        # FASE 5: NUOVA - Gestione speciale se risultato finale è "altro"
        final_label, final_method, final_confidence = fallback_result
        
        if final_label.lower() == "altro":
            self.logger.debug(f"🔍 LLM ha restituito 'altro' → Attivo BERTopic discovery")
            bertopic_discovery_result = self._try_bertopic_discovery_for_altro(conversation_text)
            if bertopic_discovery_result['found_new_pattern']:
                self.logger.info(f"✅ BERTopic discovery per 'altro': {bertopic_discovery_result['suggested_label']} "
                               f"(similarità max: {bertopic_discovery_result.get('semantic_analysis', {}).get('max_similarity', 'N/A')})")
                return (
                    bertopic_discovery_result['suggested_label'],
                    bertopic_discovery_result['method'],
                    bertopic_discovery_result['confidence']
                )
            else:
                self.logger.debug(f"❌ BERTopic discovery fallita: {bertopic_discovery_result.get('reason', 'Motivo non specificato')}")
        
        # Se non è "altro" o BERTopic non trova nulla, ritorna risultato normale
        return final_label, final_method, final_confidence
    ...
    def _try_bertopic_discovery_for_altro(self, conversation_text: str) -> Dict[str, Any]:
        """
        Ultimo tentativo con BERTopic quando il sistema classifica come 'altro'
        
        Args:
            conversation_text: Testo della conversazione da analizzare
            
        Returns:
            Dict con found_new_pattern, suggested_label, method, confidence, semantic_analysis
        """
        try:
            if not hasattr(self, 'bertopic_provider') or not self.bertopic_provider:
                return {'found_new_pattern': False, 'reason': 'BERTopic non disponibile'}
            
            self.logger.debug(f"🔍 BERTopic discovery per 'altro' su testo: '{conversation_text[:100]}...'")
            
            # 1. ANALISI BERTOPIC DEL TESTO
            topics, probabilities = self.bertopic_provider.model.transform([conversation_text])
            topic_id = topics[0]
            topic_prob = probabilities[0] if len(probabilities) > 0 else [0.0]
            max_prob = max(topic_prob) if hasattr(topic_prob, '__iter__') else topic_prob
            
            self.logger.debug(f"BERTopic analysis: topic_id={topic_id}, probability={max_prob:.3f}")
            
            # Scarta se è rumore (topic -1)
            if topic_id == -1:
                return {
                    'found_new_pattern': False,
                    'reason': 'BERTopic ha classificato come rumore',
                    'topic_id': topic_id,
                    'probability': max_prob
                }
            
            # Scarta se probabilità troppo bassa  
            if max_prob < 0.5:
                return {
                    'found_new_pattern': False,
                    'reason': f'Probabilità BERTopic troppo bassa: {max_prob:.3f}',
                    'topic_id': topic_id,
                    'probability': max_prob
                }
            
            # 2. GENERA LABEL DAL TOPIC
            topic_words = self.bertopic_provider.model.get_topic(topic_id)
            if not topic_words or len(topic_words) == 0:
                return {'found_new_pattern': False, 'reason': 'Topic senza parole chiave'}
            
            # Crea label dalle prime 3 parole più significative
            top_words = [word for word, score in topic_words[:3] if word.isalpha() and len(word) > 2]
            if len(top_words) < 2:
                return {'found_new_pattern': False, 'reason': 'Parole topic non significative'}
            
            suggested_label = '_'.join(top_words).lower()
            self.logger.debug(f"Label suggerita da BERTopic: '{suggested_label}' da parole: {top_words}")
            
            # 3. CONTROLLO SIMILARITÀ SEMANTICA CON TAG ESISTENTI
            semantic_analysis = self._analyze_semantic_similarity_with_existing(suggested_label)
            
            # 3.5. CASO SPECIALE: PRIMO AVVIO (solo "altro" disponibile)
            is_first_time_setup = len(self.domain_labels) <= 1 and 'altro' in self.domain_labels
            
            if is_first_time_setup:
                self.logger.info(f"🆕 PRIMO AVVIO: Auto-approvazione tag BERTopic '{suggested_label}' "
                               f"(probabilità: {max_prob:.3f})")
                
                # SALVA AUTOMATICAMENTE IL NUOVO TAG NEL DATABASE
                try:
                    self._add_new_validated_label(
                        suggested_label,
                        f"Tag generato automaticamente da BERTopic durante primo avvio (topic {topic_id})",
                        max_prob,
                        'BERTOPIC_AUTO_FIRST_TIME'
                    )
                    self.logger.info(f"💾 Tag '{suggested_label}' salvato automaticamente nel database")
                except Exception as e:
                    self.logger.warning(f"Errore salvataggio tag '{suggested_label}': {e}")
                
                # AUTO-APPROVE per primo avvio con alta confidenza
                return {
                    'found_new_pattern': True,
                    'suggested_label': suggested_label,
                    'method': 'BERTOPIC_ALTRO_FIRST_TIME_AUTO_APPROVE',
                    'confidence': min(0.8, max_prob),  # Confidence alta ma non troppo
            ...
            # 4. DECISIONE BASATA SU SIMILARITÀ (caso normale)
            if semantic_analysis['max_similarity'] >= self.altro_discovery_similarity_threshold:
                # TROPPO SIMILE → USA TAG ESISTENTE
                self.logger.info(f"Tag BERTopic '{suggested_label}' troppo simile a '{semantic_analysis['most_similar_tag']}' "
                               f"(similarità: {semantic_analysis['max_similarity']:.3f})")
                return {
                    'found_new_pattern': True,
                    'suggested_label': semantic_analysis['most_similar_tag'],
                    'method': 'BERTOPIC_ALTRO_SEMANTIC_MERGE',
                    'confidence': semantic_analysis['max_similarity'],
                    'semantic_analysis': semantic_analysis,
                    'bertopic_info': {
                        'topic_id': topic_id,
                        'probability': max_prob,
                        'original_suggestion': suggested_label,
                        'topic_words': [word for word, _ in topic_words[:5]]
                    }
                }
            else:
                # NUOVO TAG CANDIDATO → HUMAN REVIEW
                self.logger.info(f"Nuovo tag candidato da BERTopic: '{suggested_label}' "
                               f"(max similarità: {semantic_analysis['max_similarity']:.3f} < {self.altro_discovery_similarity_threshold})")
                return {
                    'found_new_pattern': True,
                    'suggested_label': suggested_label,
                    'method': 'BERTOPIC_ALTRO_NEW_TAG_CANDIDATE',
                    'confidence': 0.4,  # Confidence bassa per triggerare human review
                    'semantic_analysis': semantic_analysis,
                    'bertopic_info': {
                        'topic_id': topic_id,
                        'probability': max_prob,
            ...
            self.logger.error(f"Errore in BERTopic discovery per altro: {e}")
            return {'found_new_pattern': False, 'reason': f'Errore: {e}'}

    def _analyze_semantic_similarity_with_existing(self, suggested_label: str) -> Dict[str, Any]:
        """
        Analizza similarità semantica del tag proposto con tutti i tag esistenti
        
        Args:
            suggested_label: Label da confrontare
            
        Returns:
            Dict con max_similarity, most_similar_tag, all_similarities, analysis_details
        """
        if not self.embedder or not self.domain_labels:
            return {
                'max_similarity': 0.0,
                'most_similar_tag': None,
                'reason': 'Embedder o domain_labels non disponibili'
            }
        
        # Embedding del tag proposto
        suggested_embedding = self.embedder.encode_single(suggested_label.replace('_', ' '))
        
        similarities = []
        for existing_tag in self.domain_labels:
            if existing_tag == 'altro':
                continue  # Salta "altro"
                
            # Embedding tag esistente
            existing_embedding = self.embedder.encode_single(existing_tag.replace('_', ' '))
            
            # Calcola similarità coseno
            similarity = self._cosine_similarity(suggested_embedding, existing_embedding)
        ...
        most_similar_tag = similarities[0]['tag'] if similarities else None
        
        self.logger.debug(f"Similarità semantica per '{suggested_label}': "
                         f"max={max_similarity:.3f} con '{most_similar_tag}'")
        ...
        return {
            ...
            'analysis_details': {
                ...
                'threshold_used': self.altro_discovery_similarity_threshold
            }
        }
