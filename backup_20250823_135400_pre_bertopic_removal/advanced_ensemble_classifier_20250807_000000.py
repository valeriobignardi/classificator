...
class AdvancedEnsembleClassifier:
    ...
    def train_ml_ensemble(self, X_train: np.ndarray, y_train: np.ndarray) -> Dict[str, Any]:
        """
        Allena l'ensemble di classificatori ML con opzione di arricchimento tramite tag descriptions
        
        Args:
            X_train: Features di training (embeddings)
            y_train: Labels di training
            enhanced_training: Se True, arricchisce i dati con descrizioni dei tag
        ...
        training_result = {
            'training_accuracy': float(train_accuracy),  # Assicura che sia float Python
            'n_samples': int(len(X_train)),              # Converte numpy.int64 -> int
            'n_features': int(X_train.shape[1]),         # Converte numpy.int64 -> int  
            'n_classes': int(len(np.unique(y_train)))    # Converte numpy.int64 -> int
        }
    ...
    def predict_with_ensemble(self, text: str, return_details: bool = False, embedder=None) -> Dict[str, Any]:
        ...
        if self.ml_ensemble is not None:
            try:
                ...
                # Predizione
                ml_proba = self.ml_ensemble.predict_proba(embedding)[0]
        ...
        if self.ml_debugger and self.ml_debugger.enabled:
            ...
            self.ml_ensemble_debug = getattr(self, 'ml_debugger', None)
