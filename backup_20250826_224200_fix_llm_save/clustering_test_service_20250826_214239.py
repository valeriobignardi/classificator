...
class ClusteringTestService:
    ...
    def _get_pipeline(self, tenant_id: str) -> EndToEndPipeline:
        ...
        AGGIORNAMENTO 2025-08-26: Risoluzione UUID->slug per schema database
    ...
    def get_sample_conversations(self, tenant: str = 'humanitas', limit: int = 1000) -> Dict[str, Any]:
        ...
        Parametri di input:
            - tenant (str): Nome del tenant/schema database (slug)
        ...
        try:
            ...
            # 🔧 CONVERTI SLUG -> UUID per _get_pipeline
            if tenant != 'humanitas':  # Skip conversione per fallback
                tenant_uuid = self._resolve_uuid_from_slug(tenant)
                print(f"🔄 Risoluzione tenant: slug '{tenant}' -> UUID '{tenant_uuid}'")
            else:
                tenant_uuid = tenant  # Mantieni fallback per retrocompatibilità
        ...
        try:
            ...
            try:
                clusterer = HDBSCANClusterer(
                    ...
                    # Nuovi parametri per ridurre outliers
