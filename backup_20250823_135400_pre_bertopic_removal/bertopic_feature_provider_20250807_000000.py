...
Backup automatico del file TopicModeling/bertopic_feature_provider.py
Data backup: 2025-08-07
"""

# Contenuto originale di TopicModeling/bertopic_feature_provider.py al momento del backup:
...
class BERTopicFeatureProvider:
    ...
    def transform(self,
        ...
        Ritorno:
        {
          ...
          "topic_probas": np.ndarray shape (N, T) o (N, top_k),
        ...
        if top_k is not None and probs.size > 0:
            # Mantieni solo le top-k per ridurre dimensionalità
        ...
        if return_one_hot:
            ...
            max_idx = np.argmax(probs, axis=1) if T > 0 else None
            if T > 0:
                rows = np.arange(len(texts))
                one_hot[rows, max_idx] = 1.0
